...
/**
 * Created by Tomek on 2016-08-26.
 */
public class SFRenderer implements GLSurfaceView.Renderer {
    ...
    private SFEnemy[] enemies = new SFEnemy[SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS
            + SFEngine.TOTAL_WARSHIPS];
    ...
    private void initializeScouts() {
        for (int x = SFEngine.TOTAL_INTERCEPTORS; x <= SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS - 1; x++){
            SFEnemy scout;
            if (x >= (SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS) / 2) {
                scout = new SFEnemy(SFEngine.TYPE_SCOUT, SFEngine.ATTACK_RIGHT);
            } else {
    ...
    private void initializeWarships() {
        for (int x = SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS;
             x <= SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS + SFEngine.TOTAL_WARSHIPS - 1; x++){
                    ...
                    case SFEngine.TYPE_INTERCEPTOR:
                        ...
                        if (enemies[x].posY >= 3) {
                            ...
                            if (!enemies[x].isLockedOn) {
                                ...
                                enemies[x].incrementXToTarget = (enemies[x].lockOnPosX - enemies[x].posX)
